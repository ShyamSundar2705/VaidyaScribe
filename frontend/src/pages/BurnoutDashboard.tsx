    ...
    <div style={{ padding: "32px 32px 64px", maxWidth: 800 }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
          Weekly burnout risk · Doctor: {doctorId}
        </p>
      </div>
        ...
        <>
          ...
          {latest && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 28 }}>
              {[
                { label: "Burnout score", value: `${(latest.burnout_score * 100).toFixed(0)}%`, color: getBurnoutColor(latest.burnout_score) },
                { label: "Audio hours", value: `${latest.total_audio_hours.toFixed(1)}h`, color: "#0f172a" },
                { label: "Notes generated", value: latest.total_notes, color: "#0f172a" },
                { label: "Status", value: getBurnoutLabel(latest.burnout_score), color: getBurnoutColor(latest.burnout_score) },
              ].map(s => (
          ...
          {latest?.burnout_score >= 0.75 && (
            <div style={{
              ...
              High burnout risk detected this week. Consider scheduling a rest day or redistributing consultation load.
            </div>
          )}
